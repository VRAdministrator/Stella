...
func _ready() -> void: #Sets up XR headset
...
func handle_protein_scale() -> void: #Checks if both controllers are grabbing and scales selected protein accordingly
...
func handle_pointer_position() -> void: #checks if the GUI hand pointer needs to be switched and switches
...
func handle_menu_state() -> void:
	...
	if menu_open:
		MenuEntries.enable_menu(GUI)
	MenuEntries.disable_menu(GUI)
	return
