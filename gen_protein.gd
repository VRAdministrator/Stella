...
func _ready() -> void: 
	print(OS.get_system_dir(OS.SYSTEM_DIR_DOCUMENTS).get_base_dir())
...
func load_pdb(lines:PackedStringArray):
	...
	for line in lines:
		if line.substr(0,4)=="ATOM":
			...
			var temp_pos=Vector3(line.substr(32).to_float(),line.substr(40).to_float(),line.substr(48).to_float())/100.0
			atom_positions.append(temp_pos)
			center_pt+=temp_pos
			temp_atom.position=temp_pos
			var element=line.substr(77,1)
			elements.append(element)#maybe change to use single string
...
var MetalsSet:PackedStringArray=["LI", "NA", "K", "RB", "CS", "FR", "BE", "MG", "CA", "SR", "BA", "RA","AL", "GA", "IN", "SN", "TL", "PB", "BI", "SC", "TI", "V", "CR", "MN", "FE", "CO", "NI", "CU", "ZN", "Y", "ZR", "NB", "MO", "TC", "RU", "RH", "PD", "AG", "CD", "LA", "HF", "TA", "W", "RE", "OS", "IR", "PT", "AU", "HG", "AC", "RF", "DB", "SG", "BH", "HS", "MT", "CE", "PR", "ND", "PM", "SM", "EU", "GD", "TB", "DY", "HO", "ER", "TM", "YB", "LU", "TH", "PA", "U", "NP", "PU", "AM", "CM", "BK", "CF", "ES", "FM", "MD", "NO", "LR"]
...
func quary_3d(pos:Vector3,positions:PackedVector3Array,max:float,start:int)->Array:
	...
	for i in range(start+1,positions.size()):
		...
		var dx:float=absf(temp_pos.x-pos.x)
		if dx>max:continue
		var dy:float=absf(temp_pos.y-pos.y)
		if dy>max:continue
		var dz:float=absf(temp_pos.z-pos.z)
		if dz>max:continue
		var dis:float=sqrt(dx*dx+dy*dy+dz*dz)
