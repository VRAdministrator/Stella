...
public class gen_protein : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        static int[] quary_3d(Vector3 pos,Vector3[] positions,double max){
            atoms=new List<int>();
            distances=new List<double>();
            for (int i;i<len(positions);i++){
        ...
        static double pairThreshold(int i,int j){
            if (i < 0 || j < 0) return -1;
        ...
        static bond_type[] get_bonds(string[] els,Vector3[] positions){
            //stuff here
        }
        */
